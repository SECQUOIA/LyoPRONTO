...
class TestPrimaryDrying:
    ...
    def test_standard_case(self, standard_setup):
        ...
        final_dried = output[-1, 6]  # percent dried at end (0-100%)
...
flux = output[:, 5]           # kg/hr/m²
percent_dried = output[:, 6]  # 0-100% (NOT fraction!)

# Convert units for plotting
Pch_torr = Pch / 1000         # Convert mTorr → Torr
frac_dried = percent_dried / 100  # Convert percentage → fraction if needed
...
def analyze_drying_output(output):
    ...
    time_90 = np.interp(90.0, output[:, 6], output[:, 0])  # column 6 is 0-100%
    ...
    metrics['percent_dried_at_peak_flux'] = output[idx_max_flux, 6]  # 0-100%
