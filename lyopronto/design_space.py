...
def dry(vial, product, ht, Pchamber, Tshelf, dt, eq_cap, nVial):
    ...
    for i_Tsh, Tsh_setpt in enumerate(Tshelf["setpt"]):
        for i_Pch, Pch in enumerate(Pchamber["setpt"]):
            ...
            if functions.Vapor_pressure(Tsh_setpt) < Pch:
                ...
                warn(
                    f"At Tshelf={Tsh_setpt} and Pch={Pch}, sublimation is not feasible (vapor pressure < chamber pressure).", stacklevel=2
                )
            ...
            while Lck <= Lpr0:  # Dry the entire frozen product
                Rp = functions.Rp_FUN(
                    Lck, product["R0"], product["A1"], product["A2"]
                ...
                )[0]  # Sublimation front temperature array in degC
                dmdt = functions.sub_rate(
                    vial["Ap"], Rp, Tsub, Pch
                )  # Total sublimation rate array in kg/hr
                if dmdt < 0:
                    warn(
                        f"At t={t}hr, shelf temperature Tsh={Tsh} is too low for sublimation.", stacklevel=2
                    )
                    dmdt = 0.0
                Tbot = functions.T_bot_FUN(
                    Tsub, Lpr0, Lck, Pch, Rp
                )  # Vial bottom temperature array in degC
                ...
                if iStep == 0:
                    output_saved = np.array(
                        [[t, float(Tbot), dmdt / (vial["Ap"] * constant.cm_To_m**2)]]
                    )
                else:
                    output_saved = np.append(
                        output_saved,
                        [[t, float(Tbot), dmdt / (vial["Ap"] * constant.cm_To_m**2)]],
                        axis=0,
                    )
            ...
            T_max[i_Tsh, i_Pch] = np.max(
                output_saved[:, 1]
            )  # Maximum product temperature in C
            ...
            if output_saved.shape[0] <= 2:
                warn(
                    f"At Tsh={Tsh} and Pch={Pch}, drying completed in single timestep: check inputs.", stacklevel=2
                )
                sub_flux_avg[i_Tsh, i_Pch] = np.nan
                sub_flux_max[i_Tsh, i_Pch] = np.nan
                sub_flux_end[i_Tsh, i_Pch] = np.nan
            ...
            sub_flux_avg[i_Tsh, i_Pch] = np.sum(output_saved[:, 2] * del_t) / np.sum(
                del_t
            )  # Average sublimation flux in kg/hr/m^2
            sub_flux_max[i_Tsh, i_Pch] = np.max(
                output_saved[:, 2]
            )  # Maximum sublimation flux in kg/hr/m^2
            sub_flux_end[i_Tsh, i_Pch] = output_saved[
                -1, 2
    ...
    for j, Pch in enumerate([Pchamber["setpt"][0], Pchamber["setpt"][-1]]):
        ...
        while Lck <= Lpr0:  # Dry the entire frozen product
            ...
            Tsub = fsolve(
                functions.T_sub_fromTpr,
                product["T_pr_crit"],
                args=(product["T_pr_crit"], Lpr0, Lck, Pch, Rp),
            )[0]  # Sublimation front temperature array in degC
            dmdt = functions.sub_rate(
                vial["Ap"], Rp, Tsub, Pch
            )  # Total sublimation rate array in kg/hr
            ...
            dL = (
                (dmdt * constant.kg_To_g)
                * dt
                / (1 - product["cSolid"] * constant.rho_solution / constant.rho_solute)
                / (vial["Ap"] * constant.rho_ice)
                * (
                    1
                    - product["cSolid"]
            ...
            if iStep == 0:
                output_saved = np.array(
                    [[t, dmdt / (vial["Ap"] * constant.cm_To_m**2)]]
                )
            else:
                output_saved = np.append(
                    output_saved,
        ...
        if output_saved.shape[0] <= 2:
            warn(
                f"At Pch={Pch} and critical temp {product['T_pr_crit']}, drying completed in single timestep: check inputs.", stacklevel=2
            )
        ...
        del_t = output_saved[1:, 0] - output_saved[:-1, 0]
        del_t = np.append(del_t, del_t[-1])
        sub_flux_avg_pr[j] = np.sum(output_saved[:, 1] * del_t) / np.sum(
            del_t
        )  # Average sublimation flux in kg/hr/m^2
        sub_flux_min_pr[j] = np.min(
            output_saved[:, 1]
        )  # Minimum sublimation flux in kg/hr/m^2
        sub_flux_end_pr[j] = output_saved[
            -1, 1
        ]  # Sublimation flux at the end of primary drying in kg/hr/m^2
    ...
    dmdt_eq_cap = eq_cap["a"] + eq_cap["b"] * np.array(
        Pchamber["setpt"]
    )  # Sublimation rate in kg/hr
    if np.any(dmdt_eq_cap < 0):
        warn(
            "Equipment capability sublimation rate is negative for some chamber pressures; setting to nan.", stacklevel=2
        )
        # dmdt_eq_cap = np.maximum(dmdt_eq_cap, 0.0)
        dmdt_eq_cap[dmdt_eq_cap <= 0.0] = np.nan
    sub_flux_eq_cap = (
        dmdt_eq_cap / nVial / (vial["Ap"] * constant.cm_To_m**2)
    )  # Sublimation flux in kg/hr/m^2

    drying_time_eq_cap = Lpr0 / (
        (dmdt_eq_cap / nVial * constant.kg_To_g)
        / (1 - product["cSolid"] * constant.rho_solution / constant.rho_solute)
        / (vial["Ap"] * constant.rho_ice)
        * (
            1
    ...
    for k, Pch in enumerate(Pchamber["setpt"]):
        T_max_eq_cap[k] = functions.Tbot_max_eq_cap(
            Pch, dmdt_eq_cap[k], Lpr0, Lck, Rp, vial["Ap"]
        )  # Maximum product temperature in degC

    #####################################################

    return (
        np.array([T_max, drying_time, sub_flux_avg, sub_flux_max, sub_flux_end]),
        np.array(
            [
                np.array([product["T_pr_crit"], product["T_pr_crit"]]),
                drying_time_pr,
                sub_flux_avg_pr,
                sub_flux_min_pr,
                sub_flux_end_pr,
