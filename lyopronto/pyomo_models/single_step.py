...
def create_single_step_model(
    ...
    model.log_Psub = pyo.Var(
        domain=pyo.Reals, bounds=(-14, 2.5)
    )  # log(1e-6) to log(10)
    ...
    model.sublimation_rate = pyo.Constraint(
        expr=model.dmdt
        == model.Ap / model.Rp / model.kg_To_g * (model.Psub - model.Pch)
    )
    ...
    model.vial_bottom_temp = pyo.Constraint(
        expr=model.Tbot
        == model.Tsub
        + (model.Lpr0 - model.Lck)
        * model.dHs
        * model.dmdt
        * model.kg_To_g
        / model.hr_To_s
        / (model.Ap * model.k_ice)
    )
    ...
    model.temp_limit = pyo.Constraint(expr=model.Tbot <= model.T_crit)
...
def optimize_single_step(
    ...
    solution = solve_single_step(
        model, solver=solver, tee=tee, warmstart_data=warmstart_data
    )
