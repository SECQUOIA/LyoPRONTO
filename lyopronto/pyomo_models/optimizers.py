...
def create_optimizer_model(
    ...
    Args:
        ...
        use_secant_ramp_constraints (bool, default=True): When using collocation with
            ramp rate constraints, add explicit secant slope constraints in addition to
            DerivativeVar constraints. This ensures the numerical derivative between
            consecutive mesh points respects the ramp limit. Without this, collocation
            polynomial derivatives may satisfy constraints at collocation points while
            the secant slope between points can exceed the limit by ~15%. Recommended
            to keep enabled (default) for ramp-constrained collocation problems.
    ...
    if control_mode not in valid_modes:
        raise ValueError(
            f"control_mode must be one of {valid_modes}, got '{control_mode}'"
        )
    ...
    if control_mode in ["Pch", "both"]:
        ...
        if Pchamber is None:
            raise ValueError(
                f"control_mode='{control_mode}' requires Pchamber with 'min' and 'max' bounds"
            )
        ...
        if not (0.01 <= Pch_min <= 1.0):
            raise ValueError(
                f"Pchamber['min']={Pch_min} out of valid range [0.01, 1.0] Torr"
            )
        if not (0.01 <= Pch_max <= 1.0):
            raise ValueError(
                f"Pchamber['max']={Pch_max} out of valid range [0.01, 1.0] Torr"
            )
        if Pch_min >= Pch_max:
            raise ValueError(
                f"Pchamber['min']={Pch_min} must be < Pchamber['max']={Pch_max}"
            )
    else:
        # Fixed pressure mode (control_mode='Tsh') - need setpoints
        if Pchamber is None:
            raise ValueError(
                f"control_mode='{control_mode}' requires Pchamber with 'setpt' profile"
            )
        if "setpt" not in Pchamber:
            raise ValueError(
                f"control_mode='{control_mode}' requires Pchamber['setpt']"
            )
    ...
    if control_mode in ["Tsh", "both"]:
        ...
        if Tshelf is None:
            raise ValueError(
                f"control_mode='{control_mode}' requires Tshelf with 'min' and 'max' bounds"
            )
        if "min" not in Tshelf or "max" not in Tshelf:
            raise ValueError(
                f"control_mode='{control_mode}' requires Tshelf['min'] and Tshelf['max']"
            )
        ...
        if not (-50 <= Tsh_min <= 150):
            raise ValueError(
                f"Tshelf['min']={Tsh_min} out of valid range [-50, 150] °C"
            )
        if not (-50 <= Tsh_max <= 150):
            raise ValueError(
                f"Tshelf['max']={Tsh_max} out of valid range [-50, 150] °C"
            )
        if Tsh_min >= Tsh_max:
            raise ValueError(
                f"Tshelf['min']={Tsh_min} must be < Tshelf['max']={Tsh_max}"
            )
    else:
        # Fixed temperature mode (control_mode='Pch') - need setpoints
        if Tshelf is None:
            raise ValueError(
                f"control_mode='{control_mode}' requires Tshelf with 'setpt' profile"
            )
        if "setpt" not in Tshelf and "init" not in Tshelf:
            raise ValueError(
                f"control_mode='{control_mode}' requires Tshelf['setpt'] or Tshelf['init']"
            )
    ...
    def product_resistance_rule(m, t):
        ...
        return m.Rp[t] == product["R0"] + product["A1"] * m.Lck[t] / (
            1 + product["A2"] * m.Lck[t]
        )
    ...
    def kv_calc_rule(m, t):
        ...
        """
        return (
            m.Kv[t] * (1.0 + ht["KD"] * m.Pch[t])
            == ht["KC"] * (1.0 + ht["KD"] * m.Pch[t]) + ht["KP"] * m.Pch[t]
        )
    ...
    if use_finite_differences:
        ...
        discretizer.apply_to(model, nfe=n_elements, scheme="BACKWARD")
    else:
        ...
        nfe_apply = (
            int(np.ceil(max(1, n_elements) / max(1, n_collocation)))
            if treat_n_elements_as_effective
            else n_elements
        )
        discretizer.apply_to(
            model, nfe=nfe_apply, ncp=n_collocation, scheme="LAGRANGE-RADAU"
        )
    ...
    with contextlib.suppress(Exception):
        model._mesh_info = {
            ...
            "treat_effective": treat_n_elements_as_effective
            if not use_finite_differences
            else False,
        }

    # ======================
    # Secant Ramp Constraints (for collocation)
    # ======================
    # When using collocation, DerivativeVar constrains the polynomial derivative at
    # collocation points. However, the secant slope (numerical derivative) between
    # consecutive points can exceed this limit. Add explicit secant constraints
    # to ensure the actual trajectory respects the ramp limit.

    if not use_finite_differences and use_secant_ramp_constraints:
        t_points = sorted(model.t)
        n_points = len(t_points)

        # Shelf temperature secant constraints
        tsh_ramp_rate = Tshelf.get("max_ramp_rate") if Tshelf else None
        if tsh_ramp_rate is not None and tsh_ramp_rate > 0 and n_points > 1:
            # Create index set for consecutive pairs
            model.tsh_secant_idx = pyo.RangeSet(0, n_points - 2)

            def tsh_secant_up_rule(m, i):
                """Secant slope constraint: (Tsh[i+1] - Tsh[i]) / (t[i+1] - t[i]) <= ramp_rate"""
                t_i = t_points[i]
                t_ip1 = t_points[i + 1]
                dt = t_ip1 - t_i  # In normalized time
                # Constraint: (Tsh[i+1] - Tsh[i]) <= ramp_rate * t_final * dt
                return m.Tsh[t_ip1] - m.Tsh[t_i] <= tsh_ramp_rate * m.t_final * dt

            model.tsh_secant_up = pyo.Constraint(
                model.tsh_secant_idx, rule=tsh_secant_up_rule
            )

            def tsh_secant_down_rule(m, i):
                """Secant slope constraint: (Tsh[i+1] - Tsh[i]) / (t[i+1] - t[i]) >= -ramp_rate"""
                t_i = t_points[i]
                t_ip1 = t_points[i + 1]
                dt = t_ip1 - t_i
            ...
            model.tsh_secant_down = pyo.Constraint(
                model.tsh_secant_idx, rule=tsh_secant_down_rule
            )

        # Chamber pressure secant constraints
        pch_ramp_rate = Pchamber.get("max_ramp_rate") if Pchamber else None
        if pch_ramp_rate is not None and pch_ramp_rate > 0 and n_points > 1:
            model.pch_secant_idx = pyo.RangeSet(0, n_points - 2)

            def pch_secant_up_rule(m, i):
                """Secant slope constraint: (Pch[i+1] - Pch[i]) / (t[i+1] - t[i]) <= ramp_rate"""
                t_i = t_points[i]
                t_ip1 = t_points[i + 1]
                dt = t_ip1 - t_i
                return m.Pch[t_ip1] - m.Pch[t_i] <= pch_ramp_rate * m.t_final * dt

            model.pch_secant_up = pyo.Constraint(
                model.pch_secant_idx, rule=pch_secant_up_rule
            )

            def pch_secant_down_rule(m, i):
                """Secant slope constraint: (Pch[i+1] - Pch[i]) / (t[i+1] - t[i]) >= -ramp_rate"""
                t_i = t_points[i]
                t_ip1 = t_points[i + 1]
                dt = t_ip1 - t_i
            ...
            model.pch_secant_down = pyo.Constraint(
                model.pch_secant_idx, rule=pch_secant_down_rule
            )
            ...
            for idx in constr:
                try:
                    if constr[idx].equality:
                        ...
                        body_val = pyo.value(constr[idx].body)
                        lb = (
                            pyo.value(constr[idx].lower)
                            if constr[idx].lower is not None
                            else -float("inf")
                        )
                        ub = (
                            pyo.value(constr[idx].upper)
...
def add_slack_variables(
        ...
        if hasattr(model, constr_name):
            ...
            for idx in constr:
                if constr[idx].equality:
                    ...
                    model.slack_penalties.add(
                        body - model.slacks[idx, constr_name] <= target
                    )
                    model.slack_penalties.add(
                        body + model.slacks[idx, constr_name] >= target
                    )
                    ...
                    if constr[idx].upper is not None:
                        model.slack_penalties.add(
                            body <= constr[idx].upper + model.slacks[idx, constr_name]
                        )
                    if constr[idx].lower is not None:
                        model.slack_penalties.add(
                            body >= constr[idx].lower - model.slacks[idx, constr_name]
                        )
    ...
    model.obj_with_slacks = pyo.Objective(
        expr=model.t_final + slack_penalty * slack_sum, sense=pyo.minimize
    )
...
def add_control_tracking_penalty(
    ...
    if hasattr(model, "obj"):
        ...
        model.obj_with_tracking = pyo.Objective(
            expr=old_expr + tracking_weight * tracking_expr, sense=pyo.minimize
        )
    else:
        model.tracking_obj = pyo.Objective(
            expr=tracking_weight * tracking_expr, sense=pyo.minimize
        )
            ...
            for con in model.component_objects(pyo.Constraint, active=True):
                for idx in con:
                    try:
                        ...
                        lb = (
                            pyo.value(con[idx].lower)
                            if con[idx].lower is not None
                            else -float("inf")
                        )
                        ub = (
                            pyo.value(con[idx].upper)
                            if con[idx].upper is not None
                            else float("inf")
                        )
                        viol = max(0, lb - body_val, body_val - ub)
                        if viol > 1e-3:
...
def optimize_Tsh_pyomo(
    ...
    **Model Structure** (corrected Jan 2025):
                    ...
                    try:
                        ...
                        if viol > 1e-4:
                            ...
                            if violation_count <= 10:  # Only print first 10
                                print(
                                    f"  Violation in {constr.name}[{idx}]: {viol:.6f}"
                                )
                                print(
    ...
    if return_metadata:
        ...
        term = (
            str(getattr(last.solver, "termination_condition", None))
            if last is not None
            else None
        )
        iters = (
            getattr(getattr(last, "solver", None), "iterations", None)
            if last is not None
            else None
        )
        meta = {
...
def _extract_output_array(
    model: pyo.ConcreteModel, vial: Dict[str, float], product: Dict[str, float]
) -> np.ndarray:
    ...
    **Physics**: Same corrected 1 ODE + 2 algebraic as single-control optimizers
    ...
    if warmstart_scipy:
        ...
        if use_trust_region:
            ...
            reference_values["Pch"] = {
                t: Pch_scipy[scipy_indices[i]] for i, t in enumerate(sorted(model.t))
            }
            reference_values["Tsh"] = {
                t: Tsh_scipy[scipy_indices[i]] for i, t in enumerate(sorted(model.t))
            }
