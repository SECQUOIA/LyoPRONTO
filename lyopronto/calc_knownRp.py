...
def dry(vial, product, ht, Pchamber, Tshelf, dt):
    ...
    if Pch_t.max_setpt() > functions.Vapor_pressure(Tsh_t.max_setpt()):
        warn(
            "Chamber pressure setpoint exceeds vapor pressure at shelf temperature "
            + "setpoint(s). Drying cannot proceed.", stacklevel=2
        )
    ...
    if sol.t[-1] == max_t:  # and Lpr0 > sol.y[0, -1]:
        warn(
            "Maximum simulation time (specified by Pchamber and Tshelf) reached before drying completion.", stacklevel=2
        )
