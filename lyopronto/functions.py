...
def Vapor_pressure(T_sub):
    ...
    p = 2.698e10 * np.exp(
        -6144.96 / (273.15 + T_sub)
    )  # Vapor pressure at the sublimation temperature in Torr
...
def T_sub_solver_FUN(T_sub_guess, *data):
    ...
    P_sub = Vapor_pressure(
        T_sub_guess
    )  # Vapor pressure at the sublimation temperature in Torr
    Qsub = constant.dHs * (P_sub - Pch) * Ap / Rp / constant.hr_To_s  # Sublimation heat
    T_b = T_sub_guess + Qsub / Ap / constant.k_ice * (
        Lpr0 - Lck
    )  # Corresponding bottom temperature
...
def T_bot_FUN(T_sub, Lpr0, Lck, Pch, Rp):
    ...
    Tbot = (
        ...
        * constant.dHs
        ...
        / constant.hr_To_s
        / constant.k_ice
    )  # Vial bottom temperature in degC
...
def T_sub_fromTpr(T_unknown, *data):
    ...
    F = (
        ...
        * constant.dHs
        ...
        / constant.hr_To_s
        / constant.k_ice
    )  # Heat and mass transfer energy balance function - Should be zero
...
def Tbot_max_eq_cap(Pch, dm_dt, Lpr0, Lck, Rp, Ap):
    ...
    T_sub = (
        -6144.96 / np.log(P_sub / 2.698e10) - 273.15
    )  # Sublimation front temperature in degC
    Tbot = (
        T_sub
        + (Lpr0 - Lck)
        * (P_sub - Pch)
...
def Eq_Constraints(Pch, dmdt, Tbot, Tsh, Psub, Tsub, Kv, Lpr0, Lck, Av, Ap, Rp):
    ...
    C1 = Psub - 2.698e10 * np.exp(
        -6144.96 / (273.15 + Tsub)
    )  # Vapor pressure at the sublimation temperature in Torr

    C2 = dmdt - Ap / Rp / constant.kg_To_g * (Psub - Pch)  # Sublimation rate in kg/hr

    C3 = (Tsh - Tbot) * Av * Kv * (Lpr0 - Lck) - Ap * (
        Tbot - Tsub
    ) * constant.k_ice  # Vial heat transfer balance

    C4 = (
        Tsh - dmdt * constant.kg_To_g / constant.hr_To_s * constant.dHs / Av / Kv - Tbot
    ...
    def __init__(self, rampspec, count_ramp_against_dt=True):
        ...
        if "init" in rampspec:
            ...
            self.values = np.concatenate(
                ([self.setpt[0]], np.repeat(self.setpt[1:], 2))
            )
            times = np.array([0.0])
        else:
        ...
        if count_ramp_against_dt:
            for i in range(1, len(self.setpt)):
                ...
                totaltime = (
                    self.dt_setpt[min(len(self.dt_setpt) - 1, i - 1)]
                    / constant.hr_To_min
                )
                ramptime = (
                    abs((self.setpt[i] - self.setpt[i - 1]) / self.ramp_rate)
                    / constant.hr_To_min
                )
                holdtime = totaltime - ramptime
                if ramptime > holdtime:
                    warn(
                        f"Ramp time from {self.setpt[i - 1]:.2e} to {self.setpt[i]:.2e} exceeds total time for setpoint change, {totaltime}.", stacklevel=2
                    )
            ...
            for i, v in enumerate(self.setpt[1:], start=1):
                ramptime = (
                    abs((v - self.setpt[i - 1]) / self.ramp_rate) / constant.hr_To_min
                )
                # If less dt_setpt than setpt provided, repeat the last dt
                holdtime = (
                    self.dt_setpt[min(len(self.dt_setpt) - 1, i - 1)]
...
def crystallization_time_FUN(V, h, Av, Tf, Tn, Tsh_func, t0):
    ...
    hA = (
        h * constant.hr_To_s * Av * constant.cm_To_m**2
    )  # heat transfer coefficient in J/K/hr
