...
import numpy as np
import pytest
from lyopronto import constant, functions, opt_Pch
...
class TestOptPchEdgeCases:
    ...
    def test_narrow_pressure_range(self, standard_opt_pch_inputs):
        ...
        Pch_mTorr = output[:, 4]
        assert np.all(Pch_mTorr >= 70 * 0.95), (
            f"Min Pch={Pch_mTorr.min():.0f} should be >= 70 mTorr"
        )
        assert np.all(Pch_mTorr <= 90 * 1.05), (
            f"Max Pch={Pch_mTorr.max():.0f} should be <= 90 mTorr"
        )
