...
class TestOptimizerWebInterface:
    ...
    def test_optimizer_completes(self, optimizer_params):
        ...
        assert percent_dried[-1] >= PERCENT_COMPLETE, (
            f"Drying incomplete: {percent_dried[-1]:.1f}% dried"
        )
    ...
    def test_optimizer_matches_reference_timing(
        self, optimizer_params, reference_results
    ):
    ...
    def test_optimizer_matches_reference_temperatures(
        self, optimizer_params, reference_results
    ):
    ...
    def test_optimizer_matches_reference_trajectory(
        self, optimizer_params, reference_results
    ):
...
class TestOptimizerWebEdgeCases:
    ...
    def test_optimizer_different_timesteps(self, optimizer_params):
        ...
        dt_large = 0.05
        results_large = opt_Tsh.dry(
            vial, product, ht, Pchamber, Tshelf, dt_large, eq_cap, nVial
        )
        ...
        dt_small = 0.005
        results_small = opt_Tsh.dry(
            vial, product, ht, Pchamber, Tshelf, dt_small, eq_cap, nVial
        )
    ...
    def test_optimizer_different_critical_temps(self, optimizer_params):
        ...
        results_high = opt_Tsh.dry(
            vial, product_high_T, ht, Pchamber, Tshelf, dt, eq_cap, nVial
        )

        # Test with lower critical temperature (slower drying)
        product_low_T = product.copy()
        product_low_T["T_pr_crit"] = -10.0
        results_low = opt_Tsh.dry(
            vial, product_low_T, ht, Pchamber, Tshelf, dt, eq_cap, nVial
        )
