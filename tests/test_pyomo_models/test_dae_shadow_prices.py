...
@pytest.mark.parametrize(
    ("key", "overrides", "delta"),
    [
        ("product_temperature_limit", {"product": {"T_pr_crit": -24.5}}, 0.5),
        ("chamber_pressure_lower_bound", {"pchamber": {"min": 0.055}}, 0.005),
    ],
)
def test_shadow_prices_predict_the_re_solved_drying_time(
    joint_case, key, overrides, delta
) -> None:
    """Each reported sensitivity matches a finite-difference re-solve."""
    solver = require_pyomo_solver("ipopt")
    base = _solve(joint_case, solver)
    perturbed = _solve(joint_case, solver, **overrides)
