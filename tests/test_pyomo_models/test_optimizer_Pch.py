...
class TestPyomoOptPchScipyValidation:
    ...
    def test_scipy_solution_validates_on_pyomo_mesh(self, validation_params):
        ...
        residuals = validate_scipy_residuals(
            model, scipy_output, vial, product, ht, verbose=False
        )

        # Check that all residuals are small
        for name, res_dict in residuals.items():
            if "mean" in res_dict:
                assert res_dict["mean"] < 1e-3, (
                    f"{name} mean residual too large: {res_dict['mean']}"
                )
                assert res_dict["max"] < 1e-2, (
                    f"{name} max residual too large: {res_dict['max']}"
                )
...
class TestPyomoOptPchOptimization:
    ...
    def test_optimize_Pch_pyomo_converges(self, optimizer_params):
        ...
        Pch_mTorr = result[:, 4]
        assert Pch_mTorr.min() >= 60 - 5, (
            f"Pch should be >= 60 mTorr, got {Pch_mTorr.min():.1f}"
        )
        assert Pch_mTorr.max() <= 200 + 5, (
            f"Pch should be <= 200 mTorr, got {Pch_mTorr.max():.1f}"
        )
    ...
    def test_optimize_Pch_pyomo_produces_valid_solution(self, optimizer_params):
        ...
        t_pyomo / t_scipy if t_scipy > 0 else float("inf")
