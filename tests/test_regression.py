...
class TestRegressionStandardCase:
    ...
    def test_reference_initial_conditions(self, reference_case):
        ...
        assert np.isclose(
            initial_Pch_mTorr, 150.0, rtol=0.01
        )  # Chamber pressure [mTorr]
        assert np.isclose(
            initial_fraction, 0.0, atol=0.01
        )  # Starting at 0 fraction dried
    ...
    def test_reference_final_state(self, reference_case):
        ...
        assert (
            final_fraction >= PERCENT_COMPLETE
        )  # Should be essentially complete (percent)
...
class TestRegressionConsistency:
    ...
    def test_output_format_consistency(self):
        ...
        assert (
            output[-1, 6] >= PERCENT_COMPLETE
        )  # Last column is percent dried, should reach ~100%
    ...
    def test_numerical_stability(self):
        ...
        for col in range(output.shape[1]):
            ...
            if col != 6:  # Skip %dried which can have large jumps near end
                ...
                max_relative_change = np.max(
                    diffs[1:-1] / (np.abs(output[1:-2, col]) + 1e-10)
                )
