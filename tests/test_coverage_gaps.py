...
import numpy as np
import pytest
from lyopronto import design_space, functions
...
class TestDesignSpaceCoverageGaps:
    ...
    def test_design_space_shelf_temp_ramp_down(self, design_space_setup):
        ...
        """
        # Use a valid cooling ramp that still maintains feasible sublimation.
        design_space_setup['Tshelf']['init'] = -10.0
        design_space_setup['Tshelf']['setpt'] = [-20.0]
        ...
        assert len(output) == 3
        assert output[0].shape == (5, 1, 3)
        assert output[1].shape == (5, 2)
