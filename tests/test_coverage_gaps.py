...
class TestDesignSpaceCoverageGaps:
    ...
    def test_design_space_negative_sublimation(self, design_space_setup):
        ...
        assert_warning_messages(warning_record, ["sublimation is not feasible"])
    ...
    @pytest.mark.skip(
        reason="Ramp-down scenarios cause temperatures too low for sublimation, leading to numerical overflow. The ramp-down code path (lines 103-105) is tested implicitly but cannot complete physically."
    )
    ...
    def test_design_space_equipment_capability_section(self, design_space_setup):
        ...
        assert_warning_messages(warning_record, ["too low for sublimation"])

        # Equipment capability data is in output[2]
        eq_cap_data = output[2]
        assert (
            eq_cap_data.shape[0] == 3
        )  # [T_max_eq_cap, drying_time_eq_cap, sub_flux_eq_cap]
    ...
    def test_design_space_single_timestep_both_sections(self, design_space_setup):
        ...
        assert_warning_messages(warning_record, ["drying completed in single timestep"])
