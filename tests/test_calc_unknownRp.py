...
class TestCalcUnknownRpEdgeCases:
    ...
    def test_different_pressure(self, standard_inputs_nodt):
        ...
        assert np.allclose(output[:, 4], 100.0, atol=PRESSURE_ATOL), "Pch should be ~100 mTorr"
        assert_physically_reasonable_output(output)
    ...
    def test_too_long_time_series(self, standard_inputs_nodt):
        ...
        assert np.all(output[:, 6] <= PERCENT_MAX + PERCENT_ATOL), "Percent dried should not exceed 100% significantly"
